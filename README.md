...
| **Stock page `/stock/[ticker]`** | `market.instruments` + all periods of `market.performance` + `market.prices` | **Real** — name, sector, the provider's industry, country, market cap, a full performance strip and a **price chart** (1M/3M/6M/1Y). Server data wins over the deep-link params |
