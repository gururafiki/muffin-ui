...
| **Stock page `/stock/[ticker]`** | `market.instruments` + all periods of `market.performance` + `market.prices` | **Real** — name, sector, the provider's industry, country, market cap, a full performance strip and a **price chart** (1M/3M/6M/1Y), and **past agent runs for that ticker**. Server data wins over the deep-link params |
...
| `npx tsx scripts/money-check.ts` (`npm run verify:money`) | **No** — fully offline | `formatMoney`: that a figure carries the currency it is actually in. Alibaba's CNY revenue rendered as "$1.02T" — the largest company on earth by revenue, against a true ~$141B — because the page hardcoded `$`. Asserts the **locale stays pinned** (the function appends the scale suffix itself, and `de-DE` renders `215,94 $`, which the append turns into `215,94 $B`), that an unknown code cannot throw during render, that a "$" currency which is not USD is distinguishable (`MX$`), and that **no currency means no symbol** rather than dollars |
